...
def reduced_error_prunning(decisionTree, X_test, y_test):
    ...
    #decisionTree.root(decisionTree.root_node)
    leaves=get(decisionTree.root_node)
    #print(leaves)
    
    for leaf in leaves:
        #print("call")
        leaf.parent.splittable=False
        #prunning(decisionTree,leaf,X_test,y_test)
...
def prunning(decisionTree, node, X_test, y_test):
    if(node.splittable):
        if(check(node)):
            ...
            if(new_acc>decisionTree.acc and node.parent is not None):
                print("hey")
                decisionTree.accuracy=new_acc
                return prunning(decisionTree, node.parent, X_test, y_test)
            else:
                node.splittable=True
                return
        else:
...
def check(node):
    ...
    if(any(split)):
        ...
        return 1
'''    
def prune(decisionTree, node,X_test,y_test):
    split=[]
    y=y+1
    #print("Iteration",i,y)
    if node.splittable:
        ...
        if(len(node.children)!=0):
            ...
            if(not any(split)):
                ...
                if(new_acc<=decisionTree.accuracy):
                    node.splittable=True
                    node.children=ch
                else:
                    decisionTree.accuracy=new_acc
                    #node.splittable=False
                    #print("prune parent of parent")
                    prune(decisionTree,node.parent,X_test,y_test)
        else:
            #print("hello4")
...
'''
def get_leaf_nodes(decisionTree):
    leafs = []
    def _get_leaf_nodes( node):
        if node is not None:
            if(node.splittable==False):
                ...
                for n in node.children:
                    _get_leaf_nodes(n)
    _get_leaf_nodes(decisionTree.root_node)
    return leafs            
